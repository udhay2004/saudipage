  ...
  return (
    <section id="landscape" className="relative py-16 lg:py-20 bg-[#071a2b]/30 border-t border-[#c8a45d]/15 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 relative z-10">
        ...
        {/* Section Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between pb-8 sm:pb-12 mb-8 sm:mb-12 border-b border-white/10 gap-4 sm:gap-6">
          <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
            ...
            {topics.map((topic, index) => {
              ...
              return (
                <button
                  ...
                  className={`group w-full py-4 sm:py-6 text-left transition-all duration-300 flex items-center justify-between gap-3 sm:gap-4 cursor-pointer ${
                    isActive
                      ? 'pl-3 sm:pl-6 bg-[#071a2b]/60 border-l-2 border-[#c8a45d]'
                      : 'hover:pl-2 text-[#f4f1e8]/70 hover:text-white'
                  }`}
                >
                  <div className="flex items-baseline gap-3 sm:gap-6 min-w-0">
                    <span
                      className={`font-mono-tech text-xs tracking-widest transition-colors flex-shrink-0 ${
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <h3
                          className={`font-editorial text-base sm:text-2xl transition-colors truncate ${
                            isActive ? 'text-[#f4f1e8]' : 'text-[#f4f1e8]/80 group-hover:text-[#f4f1e8]'
                          }`}
                        >
                          {topic.title}
                        </h3>
                        <span className="text-[11px] font-mono-tech text-[#c8a45d]/60 opacity-80 hidden sm:inline flex-shrink-0">
                          {topic.arabicName}
                        </span>
                      </div>
                      <p className="text-[11px] sm:text-xs text-[#f4f1e8]/50 font-light mt-1 max-w-sm truncate">
                        {topic.authority}
                      </p>
                    </div>
                  </div>

                  <div className="flex items-center gap-2 pr-1 sm:pr-2 flex-shrink-0">
                    <span
          ...
          <div className="lg:col-span-6">
            <div className="sticky top-28 rounded-xl border border-[#c8a45d]/25 bg-[#05080c]/90 backdrop-blur-md p-4 sm:p-8 lg:p-10 shadow-[0_20px_60px_rgba(5,8,12,0.9)]">
              {/* Dossier Header */}
              <div className="flex items-center justify-between pb-4 sm:pb-6 border-b border-white/10 gap-2">
                <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                  <span className="font-mono-tech text-[11px] sm:text-xs tracking-widest text-[#c8a45d] font-bold">
                    MODULE {topics[selectedTopic].number}
                  </span>
                  <span className="text-white/20">|</span>
                  <span className="text-[10px] sm:text-xs font-mono-tech text-[#0b8f5a] tracking-wider uppercase">
                    Regulatory Mandate
                  </span>
                </div>
                <span className="text-xs sm:text-sm font-editorial italic text-[#c8a45d] flex-shrink-0">
              ...
              <div className="my-4 sm:my-6">
                <h4 className="font-editorial text-xl sm:text-3xl text-[#f4f1e8] mb-1.5 sm:mb-2 font-normal break-words">
                  {topics[selectedTopic].title}
                </h4>
                <div className="text-[11px] sm:text-xs font-mono-tech text-[#c8a45d]/80 tracking-wide uppercase flex items-center gap-2">
                  <Landmark className="w-3.5 h-3.5 flex-shrink-0" />
                  <span>{topics[selectedTopic].authority}</span>
                </div>
              </div>

              {/* Summary */}
              <p className="text-xs sm:text-sm text-[#f4f1e8]/80 font-light leading-relaxed mb-4 sm:mb-6">
                {topics[selectedTopic].shortSummary}
              </p>

              {/* Key Metric Callout */}
              <div className="mb-4 sm:mb-6 p-3 sm:p-4 rounded-lg bg-[#071a2b]/80 border border-[#c8a45d]/20 flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <div className="text-[10px] uppercase font-mono-tech tracking-wider text-[#c8a45d] truncate">
                    Key Regulatory Benchmark
                  </div>
                  <div className="text-xs text-[#f4f1e8]/70 font-light truncate">
                    {topics[selectedTopic].metricLabel}
                  </div>
                </div>
                <div className="font-editorial text-lg sm:text-2xl font-semibold text-[#f4f1e8] flex-shrink-0">
              ...
              <div className="space-y-2.5 sm:space-y-3">
                ...
                {topics[selectedTopic].details.map((detail, dIdx) => (
                  <div key={dIdx} className="flex items-start gap-2.5 sm:gap-3 text-xs text-[#f4f1e8]/70 font-light leading-relaxed">
              ...
              <div className="mt-6 sm:mt-8 pt-4 border-t border-white/10 flex flex-wrap items-center justify-between gap-2 text-[9px] sm:text-[10px] font-mono-tech text-[#f4f1e8]/40">
